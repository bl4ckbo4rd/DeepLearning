...
%Restricted Boltzmann Machine with continuos (Gaussian) hidden units
%We assume binary \in [0,1] variables for visible spins. 
%visible units are transformed \in [-1,1] in the script
...
function [J, Delta ] = CD_continuos(N,M,data,beta,eta,Num_samples,Num_iterations,L,F)
    ...
    for it = 1:Num_iterations
        ...
        for samp = 1:Num_samples
            ...
            h_spin(:,samp,1)   = normrnd(h_field,1,M,1); 
            ...
            h_spin(:,samp,2)   = normrnd(h_field,1,M,1);   
        end
        ...
        for samp = 1:Num_samples
            ...
            %C1 = C1 + ( v_spin(:,samp,1) * (J*v_spin(:,samp,1))' )'; 
            C1 = C1 + h_spin(:,samp,1) * v_spin(:,samp,1)'; 
